...
var mimeTypes = /*@__PURE__*/getDefaultExportFromCjs(mimeTypes$1);
...
const generalDownload = async (url) => {
    ...
    const handleDownload = () => {
        ...
        const fullPath = path.join(require$$0$2.tmpdir(), name);
    ...
    const handleFile = (pathInput, ext) => {
        return new Promise((resolve, reject) => {
            ...
            fs.rename(pathInput, fullPath, (err) => {
                if (err)
                    reject(err);
                resolve(fullPath);
            });
...
class CoreClass extends node_events.EventEmitter {
    constructor(_flow, _database, _provider, _args) {
        ...
        this.handleMsg = async (messageCtxInComming) => {
            ...
            // 📄 Finalizar flujo
            const endFlow = (flag) => async (message = null) => {
                flag.endFlow = true;
                endFlowFlag = true;
                if (message)
            ...
            const endFlowToGotoFlow = (flag) => async (messages = null, options = { fromGotoFlow: false, end: false }) => {
                flag.endFlow = true;
                endFlowFlag = true;
            ...
            const flowDynamic = (flag, inRef, privateOptions) => async (listMsg = [], options = { continue: true }) => {
            ...
            const resolveCbEveryCtx = async (ctxMessage, options = { omitEndFlow: false, idleCtx: false, triggerKey: false }) => {
                if (!!ctxMessage?.options?.idle && !ctxMessage?.options?.capture) {
                    printer(`[ATENCION IDLE]: La función "idle" no tendrá efecto a menos que habilites la opción "capture:true". Por favor, asegúrate de configurar "capture:true" o elimina la función "idle"`);
                    return;
                }
         ...
         * @param {*} ref
         */
        this.continue = (message, ref = false) => {
...
class ProviderClass extends node_events.EventEmitter {
    constructor() {
        super(...arguments);
        this.vendor = {};
    }
...
exports.MemoryDB = MemoryDBClass;
